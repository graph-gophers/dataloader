...
func (c *InMemoryCache) Set(_ context.Context, key Keyer, value Thunk) {
	c.items.Store(key.Key(), value)
}

// Get gets the value at `key` if it exsits, returns value (or nil) and bool
// indicating of value was found
func (c *InMemoryCache) Get(_ context.Context, key Keyer) (Thunk, bool) {
	item, found := c.items.Load(key.Key())
	if !found {
		return nil, false
	}
...
func (c *InMemoryCache) Delete(_ context.Context, key Keyer) bool {
	if _, found := c.items.Load(key.Key()); found {
		c.items.Delete(key.Key())
		return true
	}
	return false
}
